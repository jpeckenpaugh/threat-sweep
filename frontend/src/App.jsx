...
export default function App() {
  ...
  const [mission, setMission] = useState(null), [mode, setMode] = useState('scan'), [effects, setEffects] = useState([]), [result, setResult] = useState(null)
  ...
  const start = async (selected, restart = false) => { setPending(true); setError(''); try { const next = await api.startMission(selected.id, restart); setMission(selected); setAttempt(next); setEffects([]); setMode('scan'); setScreen('mission') } catch (err) { setError(err.message) } finally { setPending(false) } }
  ...
  const home = async () => { setScreen('home'); setAttempt(null); setResult(null); setEffects([]); await load() }
