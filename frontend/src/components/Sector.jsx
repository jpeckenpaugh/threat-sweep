...
export default function Sector({ cell, row, column, mode, onAction, pending, contaminated }) {
  const state = cell.state
  const label = `${state} sector ${row + 1}, ${column + 1}${cell.revealedThreat ? `, ${cell.revealedThreat}` : ''}`
  return <button disabled={pending || state === 'cleared'} onClick={() => onAction(row, column)} className={`sector ${state} ${cell.revealedThreat ? 'threat-revealed' : ''} ${contaminated ? 'contaminated' : ''}`} aria-label={label}>
